...
- Immediate compensate-and-continue fallback for zero-bit domain/simulator failures.
...
- Before materialization or measured execution, catalog validation rejects dangling or misowned WorkloadPlan slots/checkpoints, duplicate action identities, actions referencing absent forward steps, compensation actions before their enabling failure/completion, participant-order violations, and residual forward-order violations.
...
- The executor follows the selected FaultScenario action order exactly unless a zero-bit domain/simulator failure causes the defined fallback.
...
- A domain/simulator exception during close/commit is `UNASSIGNED_RUNTIME` at the final forward action: the participant does not commit, immediate fallback compensation runs, and still-valid actions for other participants continue. Reflection, provider, configuration, or invocation-shape failures during closure are executor/infrastructure hard stops.
- An assigned fault in a participant's final forward action prevents its body and commit from running.
- A zero-bit domain or simulator exception is recorded with `failureOrigin = UNASSIGNED_RUNTIME`. The participant enters one immediate fallback recovery episode through the simulator's normal compensation path, its remaining forward actions are skipped, and still-valid planned actions for other participants continue. "Once" means one recovery episode, not one opaque compensation action.
...
- Eager and on-demand FaultScenario generation may add missing deterministic records to the fault-scenario artifact for that run. A successful generation request updates the FaultScenario JSONL, manifest, and accounting as one consistent package revision; validation or write failure leaves the prior package byte-unchanged.
...
- [ ] AC-20: An explicit on-demand request for one WorkloadPlan whose participant inputs are ready and whose plan shape is structurally admissible, plus one valid arbitrary binary vector, persists bounded deterministic FaultScenarios before execution and supports multiple assigned bits; runtime materialization/startup remain execution gates.
- [ ] AC-21: Invalid/missing WorkloadPlan ids, non-binary or wrong-length vectors, malformed slot mappings, structurally inadmissible plans, and non-ready participant inputs fail with structured diagnostics before catalog mutation or execution, leaving FaultScenario JSONL, manifest, and accounting byte-unchanged.
- [ ] AC-22: Repeating an on-demand request with the same semantic inputs deduplicates byte-equivalent existing FaultScenarios rather than appending duplicate ids; an existing id with different semantic content is an integrity error and leaves the package unchanged.
...
- [ ] AC-25: V3 execution selects one persisted FaultScenario id, rejects direct unpersisted execution-time vector overlays, and validates action references, ownership, uniqueness, enabling conditions, reverse compensation order, and residual global forward order before materialization or measured execution.
...
- [ ] AC-31: A zero-bit domain/simulator exception from a forward body or close/commit is recorded as `UNASSIGNED_RUNTIME`, starts one immediate fallback recovery episode, skips that participant's remaining forward actions, and continues still-valid actions for other participants; checkpoint-level recovery follows runtime unit-of-work truth and may emit a runtime-only source-step reference for implicit rollback of the partially failed step.
...
- The simulator's current stepwise compensation and unit-of-work bookkeeping remain the runtime source of truth for whether a compensation invocation has observable work.
