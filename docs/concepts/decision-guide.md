...
These are added by `/scaffold-aggregate` (Step 3) when processing the aggregate's plan.md entry.
...
(including its cached snapshot fields)?
  YES → Layer 1 — implement in verifyInvariants() (added by /scaffold-aggregate)
  NO  → continue ↓
...
Must the rule be checked SYNCHRONOUSLY (strong consistency — same UoW)?
  YES → Does the check read only the aggregate being mutated?
    ...
    NO  → Layer 3 — saga step with setForbiddenStates, wired by /implement-functionality
  NO  → Eventually consistent (~1 s lag) is acceptable?
    YES → Layer 4 — inter-invariant, wired by /wire-event
```
...
- If blocking must be synchronous → **Layer 3** (saga step with `setForbiddenStates`, wired by `/implement-functionality`).
- If true eventual consistency is acceptable → **Layer 4** (inter-invariant, wired by `/wire-event`); no guard is added.
...
|--------------------------|-------|----------------|
| All referenced data is already inside the same aggregate (including snapshots) | 1 | intra-invariant (`/scaffold-aggregate`) |
| Synchronous check, reads only the mutated aggregate's own table | 2 | service guard (`/implement-functionality`) |
| Synchronous check, reads a DIFFERENT aggregate | 3 | saga step with `setForbiddenStates` (`/implement-functionality`) |
| Eventual cache sync from another aggregate (no blocking) | 4 | inter-invariant (`/wire-event`) |
