...
class DeleteUserCompensationTest extends QuizzesFullSpockTest {
    ...
    def "deleteUser: fault on deleteUserStep compensates the lock acquired by getUserStep"() {
        ...
        // the time deleteUserStep's injected fault fires, so this exercises the saga's own
        ...
        and: 'the mutation never ran: user is still readable, not deleted'
        def reread = userFunctionalities.getUserById(userDto.aggregateId)
        reread.aggregateId == userDto.aggregateId
