...
| Scenario execution participant | One saga instance participating in a scenario execution attempt. In the proposed v3 execution report, participant entries carry saga-specific identity, materialization/startup state, lifecycle outcome, scheduled-step outcomes, skipped steps, and saga-local blockers. |
| Deterministic interleaving replay | Planned multi-saga ScenarioExecutor behavior where the executor follows a catalog `ScenarioPlan.expandedSchedule` in ascending `scheduleOrder`, stepping the owning saga instance for each scheduled step. This is sequential replay of a static interleaving, not true parallel/threaded execution or distributed runtime parity. |
| Compensate-and-continue | Proposed multi-saga assigned-fault policy: when one saga instance realizes an assigned fault, compensate that failed saga immediately when possible, skip its remaining scheduled forward steps, continue other saga instances in catalog schedule order, and close surviving saga instances at the end. |
