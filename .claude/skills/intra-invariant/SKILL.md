...
> If this rule depends on state from another aggregate, consider Layer 6 (`/inter-invariant`) instead. If the rule only applies at the moment of a specific mutation, consider Layer 2 (aggregate mutation guard in `remove()` or a setter). Consult `docs/concepts/invariants.md` if uncertain.
