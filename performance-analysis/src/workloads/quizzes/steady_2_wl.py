...
class SteadyWorkload(Workload):
    ...
    def browse_tournaments_and_quizzes(self):
        """Touches: tournament, quiz"""
        ...
        if SteadyWorkload.active_quizzes:
            _, quiz_data = random.choice(SteadyWorkload.active_quizzes)
            if "aggregateId" in quiz_data:
                SimInterface.get_quiz(
                    quiz_data["aggregateId"], client=self.client)
    ...
    def teacher_creates_quiz(self):
        """Touches: question, quiz, execution"""
    ...
    def teacher_creates_tournament(self):
        """Touches: tournament, execution, topic, question, quiz (Sagas)"""

        tourn = SimInterface.create_tournament(
            self.exec_id, [self.topic_id], self.owner_id, client=self.client)
        if tourn and "aggregateId" in tourn:
            self.tourn_id = tourn["aggregateId"]

    def student_solves_tournament_quiz(self):
        """Touches: tournament, execution, user, quiz, answer, question (Sagas)"""
    ...
    def student_solves_quiz(self):
        """Touches: execution, quiz, answer, question"""

        if not SteadyWorkload.active_quizzes:
            # Fallback: create a quiz if none exists yet
            questions = SimInterface.get_questions_for_course(
                self.course_id, client=self.client)
            if questions and len(questions) > 0:
                quiz = SimInterface.create_quiz(
                    self.exec_id, questions=questions[:2], client=self.client)
                if quiz and "aggregateId" in quiz:
                    SteadyWorkload.active_quizzes.append((self.exec_id, quiz))
    ...
    def dynamic_router(self):
        tasks = [
            ...
            self.browse_tournaments_and_quizzes,
            self.teacher_creates_question,
            self.teacher_creates_quiz,
            self.teacher_creates_tournament,
            self.student_solves_tournament_quiz,
