...
public class UpdatePriceConfigFunctionalitySagas extends WorkflowFunctionality {
    ...
    public UpdatePriceConfigFunctionalitySagas(SagaUnitOfWorkService unitOfWorkService,
                                               Integer priceConfigAggregateId,
                                               PriceConfigDto updatedPriceConfigDto,
                                               SagaUnitOfWork unitOfWork, CommandGateway commandGateway) {
        this.unitOfWorkService = unitOfWorkService;
        this.commandGateway = commandGateway;
        buildWorkflow(priceConfigAggregateId, updatedPriceConfigDto, unitOfWork);
    }

    public void buildWorkflow(Integer priceConfigAggregateId, PriceConfigDto updatedPriceConfigDto,
        ...
        SagaStep getPriceConfigStep = new SagaStep("getPriceConfigStep", () -> {
            ...
            sagaCommand.setSemanticLock(PriceConfigSagaState.IN_UPDATE_PRICE_CONFIG);
        ...
        SagaStep updatePriceConfigStep = new SagaStep("updatePriceConfigStep", () -> {
            UpdatePriceConfigCommand cmd = new UpdatePriceConfigCommand(
                    unitOfWork, ServiceMapping.PRICE_CONFIG.getServiceName(),
                    priceConfigAggregateId, updatedPriceConfigDto);
            commandGateway.send(cmd);
        }, new ArrayList<>(Arrays.asList(getPriceConfigStep)));

        this.workflow.addStep(getPriceConfigStep);
        this.workflow.addStep(updatePriceConfigStep);
    }
