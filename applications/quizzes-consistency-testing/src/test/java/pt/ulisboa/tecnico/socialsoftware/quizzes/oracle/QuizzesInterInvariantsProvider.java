...
public class QuizzesInterInvariantsProvider implements InterInvariantsProvider {
    ...
    public Set<InterInvariant> getInterInvariants() {
        return Set.of(
                ...
                invariant("MAX_TOURNAMENTS_PER_USER", this::tournamentParticipationRespectsQuota));
    }
    ...
    private boolean tournamentParticipationRespectsQuota(Snapshot state) {
        Map<ExecutionUser, Long> participationCounts = active(state.tournaments).stream()
                .flatMap(tournament -> tournament.getTournamentParticipants().stream()
                        .filter(participant -> participant.getState() == ACTIVE)
                        .map(participant -> new ExecutionUser(
                                tournament.getTournamentCourseExecution().getCourseExecutionAggregateId(),
                                participant.getParticipantAggregateId())))
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
        return participationCounts.values().stream()
                .allMatch(count -> count <= MAX_TOURNAMENTS_PER_USER);
    }
