...
public class TutorialExceptionHandler {
    private static final Logger logger = LoggerFactory.getLogger(TutorialExceptionHandler.class);

    @ExceptionHandler(TutorialException.class)
    public ResponseEntity<String> handleTutorialException(TutorialException ex) {
        logger.warn("Handled TutorialException: {}", ex.getErrorMessage());
