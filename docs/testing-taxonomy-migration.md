...
| - [x] 3.2 | User | `UserServiceTest` | `DeleteUserEvent`, `UpdateStudentNameEvent`, `AnonymizeStudentEvent` + negative case — `UserEventPublicationTest.groovy` deleted, methods merged into `UserServiceTest.groovy` (amendment retrofit, see decision 8) | `CreateUserTest`, `DeleteUserTest`, `UpdateUserNameTest`, `UpdateStudentNameTest`, `AnonymizeUserTest`, `AnonymizeStudentTest`, `GetUserByIdTest` | Retrofitted post-merge: `DeleteUserCompensationTest`, `UpdateUserNameCompensationTest`, `AnonymizeUserCompensationTest` |
...
2. ~~**Phase-3 table (lines 131-140) is stale relative to what actually happened.**~~ **Resolved
   2026-07-11.** Row 3.2's T4 trim-scope cell no longer lists `GetStudentByExecutionIdAndUserIdTest`
   (it was left untouched in 3.2, per the Discovered issues log); row 3.4's cell now includes
   `GetStudentByExecutionIdAndUserIdTest`, `UpdateStudentNameTest`, `AnonymizeStudentTest` (where
   those cases actually landed); row 3.8's cell now includes
   `sagas/coordination/tournament/SolveQuizTest`. All other cells (event-publication columns, the
   3.6/3.7 `ConcludeQuizTest`/`SolveQuizTest` handling, the 3.7 producer-side cross-reference
   wording, the 3.8 compensation-test caveat) already matched the log and needed no change.

3. ~~**`DateHandler.now()` timezone gotcha not yet promoted into `docs/concepts/testing.md`.**~~
   **Resolved 2026-07-11.** Folded into the "Temporal mechanics" paragraph under § Choosing Input
   Values — EP & BVA.
