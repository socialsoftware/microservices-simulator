...
export class NebulaValidator {
  ...
  private validateJpqlQuery(
    ...
    const syntaxErrors = this.validateJpqlSyntax(cleanQuery);
  ...
  private validateJpqlSyntax(query: string): Array<{ message: string }> {
    ...
    while ((match = keywordPattern.exec(queryUpper)) !== null) {
      ...
      if (validKeywords.has(word) || /^\d+$/.test(word) || word.length <= 2) {
        continue;
      }
      ...
      if (word === originalWord.toUpperCase() && word.length >= 3 && !foundInvalidKeywords.has(word)) {
        const similarKeyword = this.findSimilarKeyword(word, Array.from(validKeywords));
        if (similarKeyword) {
          errors.push({
  ...
  private findSimilarKeyword(word: string, validKeywords: string[]): string | null {
