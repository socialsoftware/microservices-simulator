package pt.ulisboa.tecnico.socialsoftware.showcase.microservices.room.commandHandler;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.cloud.stream.function.StreamBridge;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Profile;
import org.springframework.messaging.Message;
import org.springframework.stereotype.Component;
import pt.ulisboa.tecnico.socialsoftware.ms.coordination.workflow.command.Command;
import pt.ulisboa.tecnico.socialsoftware.ms.coordination.workflow.command.MessagingObjectMapperProvider;
import pt.ulisboa.tecnico.socialsoftware.ms.coordination.workflow.command.stream.StreamCommandHandler;

import java.util.function.Consumer;

@Component
@Profile("stream")
public class RoomStreamCommandHandler extends StreamCommandHandler {

    private final RoomCommandHandler roomCommandHandler;

    @Autowired
    public RoomStreamCommandHandler(StreamBridge streamBridge,
            RoomCommandHandler roomCommandHandler,
            MessagingObjectMapperProvider mapperProvider) {
        super(streamBridge, mapperProvider);
        this.roomCommandHandler = roomCommandHandler;
    }

    @Override
    protected String getAggregateTypeName() {
        return "Room";
    }

    @Override
    protected Object handleDomainCommand(Command command) {
        return roomCommandHandler.handleDomainCommand(command);
    }

    @Bean
    public Consumer<Message<?>> roomServiceCommandChannel() {
        return this::handleCommandMessage;
    }
}
