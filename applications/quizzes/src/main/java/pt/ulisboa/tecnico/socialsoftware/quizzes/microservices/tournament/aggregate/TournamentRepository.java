...
public interface TournamentRepository extends JpaRepository<Tournament, Integer> {
    ...
    @Query(value = "select t1 from Tournament t1 where t1.aggregateId = :aggregateId and t1.version = (select max(t2.version) from Tournament t2 where t2.aggregateId = :aggregateId)")
    Optional<Tournament> findAnySagaAggregate(Integer aggregateId);
}
