...
- AC-9: Materialized `ScenarioPlan.expandedSchedule` rows produced by `SEGMENT_COMPRESSED` match the accounting schedule count for small fully materialized fixtures when no global `maxCatalogScenarios` cap truncates the catalog.
