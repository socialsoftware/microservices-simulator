...
| Scenario execution | Runtime capability/stage that materializes catalog inputs and executes a scenario plan schedule. The current implementation supports a narrow saga/local fault-vector path for materializable single-saga plans and explicit materializable multi-saga deterministic interleavings, not arbitrary catalog replay. |
...
| Scenario execution participant | One saga instance participating in a scenario execution attempt. In the v3 execution report, participant entries carry saga-specific identity, materialization/startup state, lifecycle outcome, scheduled-step outcomes, skipped steps, and saga-local blockers. |
| Deterministic interleaving replay | Implemented bounded multi-saga ScenarioExecutor behavior where the executor follows a catalog `ScenarioPlan.expandedSchedule` in ascending `scheduleOrder`, stepping the owning saga instance for each scheduled step. This is sequential replay of a static interleaving, not true parallel/threaded execution or distributed runtime parity. |
| Compensate-and-continue | Implemented bounded multi-saga scheduled-step failure policy: when one saga instance realizes an assigned fault or throws a scheduled-step runtime failure, compensate that failed saga immediately when possible, skip its remaining scheduled forward steps, continue other saga instances in catalog schedule order, and close surviving saga instances at the end. |
| Deferred-compensation continuation | Deferred experimental multi-saga policy where a failed saga's compensation is intentionally delayed while other saga instances continue. It may expose delayed-recovery impacts, but it is not the selected first multi-saga executor policy. |
...
| ScenarioExecutor supported path | The currently implemented bounded runtime slice: one materializable saga/local scenario execution attempt per run, with either the plan default vector or one explicit binary fault vector, producing a standalone v3 participant report. Single-saga plans may be auto-selected or explicitly selected; multi-saga plans require explicit selection and are replayed as deterministic sequential interleavings. |
