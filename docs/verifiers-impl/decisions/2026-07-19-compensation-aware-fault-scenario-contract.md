...
Implementation note (2026-07-20): current generation publishes the five-file WorkloadPlan/FaultScenario v3 package, eager materializable all-zero/single-point vectors, exact bounded recovery accounting, and atomic on-demand multi-fault revisions. Current execution selects one persisted FaultScenario with no vector overlay and writes an action-aware v4 report. The bounded Quizzes evidence is recorded in [`../evidence.md`](../evidence.md).
