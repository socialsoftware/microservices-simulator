...
- `catalogWriteMode=WRITE_PLANS` materializes selected `ScenarioPlan` rows subject to `maxCatalogScenarios`; that cap affects `catalogWritten`/manifest export counts but not `allInputBound` or `selectedByGenerator` accounting totals.
