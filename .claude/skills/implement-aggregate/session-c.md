...
6. **`docs/concepts/testing.md`** — § T3 — Functionality Test (the read-functionality assertions, plus § Not-Found Assertions for the Path A / Path B rule of thumb). T1 (intra-invariant) and T4 (inter-invariant) tests are not produced in this session.
...
### One `{Query}Test.groovy` per read functionality (T3)
