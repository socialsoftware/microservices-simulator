...
2. **`docs/concepts/testing.md`** — § T4 — Inter-Invariant Test and § T4 Deletion-Event Tests. Note:
   - What a T4 test asserts (event received → cached field updated → invariant re-evaluated)
...
In the corresponding T4 test, the "ignores unrelated" scenario can use a second entity whose anchor ID is never registered with the consumer aggregate — no extra setup is needed.
