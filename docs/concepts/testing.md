...
| **T2 Functionality** | `<FunctionalityName>Test` | Happy path · P3 guard violations · ≥1 step-interleaving case per saga step boundary |
...
**Purpose:** Cover the happy path, P3 guard violations, and the concurrent-interleaving cases that
validate semantic locks at each saga step boundary. P1 intra-invariants are **not** tested here —
see § T1.
