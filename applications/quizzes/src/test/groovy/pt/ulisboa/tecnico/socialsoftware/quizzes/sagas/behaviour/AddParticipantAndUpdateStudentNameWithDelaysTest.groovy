...
@DataJpaTest
class AddParticipantAndUpdateStudentNameWithDelaysTest extends QuizzesSpockTest {
    ...
    // def 'sequential: update delay; add' () {
    //     given: 'a clear report'
    //     behaviourService.cleanReportFile()

    //     and: 'one functionality to update a student\'s name'
    //     def updateNameDto = new UserDto()
    //     updateNameDto.setName(UPDATED_NAME)
    //     def updateStudentNameFunctionality = new UpdateStudentNameFunctionalitySagas(
    //         courseExecutionService, courseExecutionFactory, unitOfWorkService,
    //         courseExecutionDto.getAggregateId(), userDto.getAggregateId(),
    //         updateNameDto, unitOfWork1
    //     )

    //     and: 'one functionality to add a participant'
    //     def addParticipantFunctionality = new AddParticipantFunctionalitySagas(
    //         tournamentService, courseExecutionService, unitOfWorkService,
    //         tournamentDto.getAggregateId(), courseExecutionDto.getAggregateId(),
    //         userDto.getAggregateId(), unitOfWork2
    //     )

    //     and: 'a student name is updated'
    ...
    //     def definedDelay = updateStudentNameFunctionality.getWorkflowTotalDelay()
    ...
    //     and: 'the execution duration of updateStudentNameFunctionality is bigger than the defined delay'
    ...
    def 'concurrent: add-step1; update; add-step2; event' () {
        given: 'a clear report'
        behaviourService.cleanReportFile()

        and: 'one functionality to add a participant that executes the first step'
        def addParticipantFunctionality = new AddParticipantFunctionalitySagas(
            tournamentService, courseExecutionService, unitOfWorkService, 
            tournamentDto.getAggregateId(), courseExecutionDto.getAggregateId(), 
            userDto.getAggregateId(), unitOfWork2)
        ...
        and: 'executor for concurrency'
        def executor = Executors.newFixedThreadPool(2)

        when: 'the student name is updated, the addparticipant executes second step concurrently'
        def updateFuture = executor.submit {
            updateStudentNameFunctionality.executeWorkflow(unitOfWork1)
        }

        def resumeFuture = executor.submit {
