...
@DataJpaTest
@Transactional
@Import(AnonymizeUserTest.LocalBeanConfiguration)
class AnonymizeUserTest extends QuizzesFull2SpockTest {

    @Autowired
    LocalCommandGateway commandGateway

    def "anonymizeUser: success"() {
        // Spec: plan.md §2 User — AnonymizeUser(userAggregateId)
    ...
    def "anonymizeUser: getUserStep acquires IN_ANONYMIZE_USER semantic lock"() {
        // Spec: plan.md §2 User — saga state IN_ANONYMIZE_USER acquired by the primary lock step
        given:
        def userAggregateId = createUser()
        ...
        expect: 'acquire transition: NOT_IN_SAGA → IN_ANONYMIZE_USER'
        sagaStateOf(userAggregateId) == UserSagaState.IN_ANONYMIZE_USER
