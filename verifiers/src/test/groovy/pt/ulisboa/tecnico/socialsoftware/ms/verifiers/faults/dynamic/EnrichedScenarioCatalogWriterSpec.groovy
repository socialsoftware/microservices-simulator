...
class EnrichedScenarioCatalogWriterSpec extends Specification {
    ...
    def 'writes WorkloadPlan linked sidecar records without embedding semantic catalogs'() {
        given:
        def root = Files.createTempDirectory('enriched-catalog-writer')
        def plan = workloadPlan('scenario-1')
        def joinResult = new DynamicEvidenceJoiner().join([plan], [])
        def writer = new EnrichedScenarioCatalogWriter()
        def catalogPath = root.resolve('workload-dynamic-evidence.jsonl')
        def manifestPath = root.resolve('workload-dynamic-evidence-manifest.json')
        def reportPath = root.resolve('dynamic-evidence-join-report.json')

        when:
        writer.write(joinResult, catalogPath, manifestPath, reportPath, 'workload-catalog.jsonl', 'dynamic-evidence', [:], [], '2026-05-01T00:00:00Z')

        then:
        def lines = Files.readAllLines(catalogPath)
        lines.size() == 1
        def enriched = mapper.readTree(lines[0])
        enriched.path('schemaVersion').asText() == WorkloadDynamicEvidenceRecord.SCHEMA_VERSION
        enriched.path('workloadPlanId').asText() == 'scenario-1'
        enriched.path('inputVariantIds').collect { it.asText() } == [plan.acceptedInputs()[0].deterministicId()]
        !enriched.has('workloadPlan')
        !enriched.has('faultScenario')
        !enriched.has('forwardSchedule')
    ...
    def 'writes manifest and join report counts'() {
        ...
        def joinResult = new DynamicEvidenceJoiner().join([
                workloadPlan('covered'),
                workloadPlan('not-covered')
        ], [])
        def writer = new EnrichedScenarioCatalogWriter()
        def catalogPath = root.resolve('workload-dynamic-evidence.jsonl')
        def manifestPath = root.resolve('workload-dynamic-evidence-manifest.json')
        ...
        when:
        writer.write(joinResult, catalogPath, manifestPath, reportPath, 'workload-catalog.jsonl', 'dynamic-evidence', [enabled: true], testRuns, '2026-05-01T00:00:00Z')
        ...
        manifest.path('sourceWorkloadCatalogPath').asText() == 'workload-catalog.jsonl'
        manifest.path('dynamicEvidenceRoot').asText() == 'dynamic-evidence'

        and:
        def report = mapper.readTree(Files.readString(reportPath))
        report.path('schema').asText() == 'microservices-simulator.dynamic-evidence-join-report.v3'
        ...
        report.path('counts').path('workloadPlansRead').asInt() == 2
        report.path('counts').path('workloadPlansEnriched').asInt() == 2
        report.path('counts').path(DynamicEvidenceJoinStatus.NOT_COVERED.name()).asInt() == 2
    }

    private static WorkloadPlan workloadPlan(String id) {
