...
public class ActivateUserFunctionalitySagas extends WorkflowFunctionality {
    ...
    public ActivateUserFunctionalitySagas(SagaUnitOfWorkService unitOfWorkService, Integer userAggregateId,
    ...
    public void buildWorkflow(SagaUnitOfWorkService unitOfWorkService, Integer userAggregateId,
        ...
        SagaStep getUserStep = new SagaStep("getUserStep", () -> {
            ...
            sagaCommand.setSemanticLock(UserSagaState.IN_ACTIVATE_USER);
            this.userDto = (UserDto) commandGateway.send(sagaCommand);
        });

        SagaStep activateUserStep = new SagaStep("activateUserStep", () -> {
            ActivateUserCommand cmd = new ActivateUserCommand(
                    unitOfWork, ServiceMapping.USER.getServiceName(), userAggregateId);
            commandGateway.send(cmd);
        }, new ArrayList<>(Arrays.asList(getUserStep)));

        this.workflow.addStep(getUserStep);
        this.workflow.addStep(activateUserStep);
    }
