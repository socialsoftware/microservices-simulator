...
| Scheduled step | A forward step occurrence in a WorkloadPlan's global forward schedule, including its saga instance and deterministic occurrence identity. Occurrence ids remain semantic catalog/action identities. Current Saga/local materializability additionally requires `runtimeStepName` to be unique within each participant because runtime state is still keyed by step name; separate participants may use the same runtime name. |
...
| Domain failure | Meaningful application or Saga transactional rejection explicitly marked by the simulator `DomainFailure` contract. `QuizzesException` and `SimulatorDomainException` are current Saga/local examples. A plain `SimulatorException` is not a domain failure merely because of its base class. |
| Infrastructure failure | Any failure without the explicit domain marker, including unknown/plain `SimulatorException`, ordinary runtime failures, service unavailability, missing execution infrastructure, and leaked assigned-fault exceptions. The supported executor conservatively hard-stops, runs no fallback, and does not continue survivors. |
| Unassigned runtime fallback | Deterministic deviation policy only for a zero-bit explicitly domain-marked body or commit failure: run one immediate checkpoint-level recovery episode for that participant, skip its remaining forward actions, and continue still-valid actions for other participants while reporting planned and actual order. |
...
| Term | Meaning |
|---|---|
| Static recipe readiness | Input-level accounting that asks whether the extracted static recipe itself is complete enough for materialization without executor-owned/runtime-owned argument resolution. In the post-event Quizzes count-only run this is `0`, distinct from accepted input coverage and executor setup candidacy. |
| Manifest-declared setup candidate | A WorkloadPlan whose existing v3 `workloadMaterializability.materializable` row is `true` because its accepted inputs pass static executor-readiness checks and its structure is admissible. Eager FaultScenario generation uses this deterministic static gate. Despite the legacy field name, actual argument materialization and Saga startup remain unproven until setup preflight or normal execution runs. |
| ScenarioExecutor setup preflight | Optional batch operation that selects manifest-declared setup candidates, uses one application context and the normal executor setup implementation to materialize each exact persisted tuple and start each exact Saga, then stops before every forward, fault, compensation, or commit action. `SETUP_READY` is concrete evidence of ScenarioExecutor materializability in that runtime; `MATERIALIZATION_FAILED` or `STARTUP_FAILED` is a false positive from the static candidate gate. It predicts neither domain success nor fault outcome. |
| ScenarioExecutor materializability | The runtime setup property established when the executor can materialize every participant's exact persisted tuple and start every exact Saga. A preflight `SETUP_READY` result or the same successful setup inside normal execution proves it for that attempt. It does not mean an all-zero or faulty execution will complete successfully. |
...
| Immediate checkpoint fallback | Zero-bit explicitly domain-marked failure policy: recover that participant immediately from runtime checkpoint truth, skip its remaining forward actions, continue valid survivor actions, and report `DEVIATED`. Unknown or unmarked failures do not use this policy. It does not rewrite the persisted FaultScenario. |
...
| Invariant-violation signal | Structured `INVARIANT_VIOLATION` dynamic-evidence event emitted only when the existing `Aggregate.verifyInvariants()` call rejects a Saga write in `SagaUnitOfWorkService.registerChanged`. One rejected invocation emits one signal and rethrows the original exception unchanged. |
| ImpactV1 | First implemented application-independent impact model: `impactScore = invariantViolationCount`. Assigned faults, Saga aborts, and compensation activity do not contribute by themselves. |
| Scenario impact report | Optional `microservices-simulator.scenario-impact-report.v1` sidecar linked to one v4 execution attempt, WorkloadPlan, and FaultScenario. It retains ordered invariant findings without changing the v4 execution report or five-file package. |
| Impact evaluation status | `EVALUATED` only for completed `SUCCESS`, `COMPENSATED`, or `PARTIAL_COMPENSATED` execution outcomes. Setup, infrastructure, compensation, dry-run, and report-write failures are `NOT_EVALUATED` with null count/score rather than false zero or positive fitness. |
| Domain-impact scoring | Incremental scoring based on domain-visible outcomes. ImpactV1 currently covers only application-declared Saga invariant rejections; compensation divergence, inconsistent final state, silent postcondition failures, and business-state anomalies remain future models. |
