...
@DataJpaTest
class RemoveStudentFromCourseExecutionTest extends QuizzesSpockTest {
    @Autowired
    ...
    def "remove student successfully"() {
        when:
    ...
    def "remove non-enrolled student throws exception"() {
        given: 'a user not enrolled in the course execution'
        def unenrolledUser = createUser(USER_NAME_2, USER_USERNAME_2, STUDENT_ROLE)

        when:
        courseExecutionFunctionalities.removeStudentFromCourseExecution(courseExecutionDto.getAggregateId(), unenrolledUser.getAggregateId())

        then:
        thrown(QuizzesException)
    }

    def "remove one of two students successfully"() {
        given: 'a second enrolled student'
