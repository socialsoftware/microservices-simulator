...
The manifest path, WorkloadPlan id, and vector are required. The request uses the package recovery cap by default; a supplied positive cap becomes that request's effective cap. An exact workload/vector/cap repeat deduplicates, while a different cap may add only schedules not already present. New additions are also deduplicated against the exact compact executable content already persisted: WorkloadPlan id, fault vector, and the complete ordered `step`/`event`/`compensate` references. This comparison deliberately excludes only the FaultScenario id. The WorkloadPlan identity therefore continues to preserve setup, participants, fault slots, event origins, and recovery/semantic-lock metadata; different action kinds, references, or orders remain distinct. Existing IDs and historical duplicate records are not rewritten. If a retained package already contains multiple IDs for equal executable content, a new request references the lexicographically lowest retained ID. The result status is `PERSISTED`, `DEDUPLICATED`, `REJECTED`, `INTEGRITY_FAILURE`, or `PERSISTENCE_FAILED`; only the first two are successful.
