...
**Updated note on delete pattern (retracted above — see retro-2.4.b-Execution):** Copy-on-write is the canonical soft-delete pattern. In-place mutation (`remove()` on a managed JPA entity before `registerChanged`) allows JPA to auto-flush the dirty state before the saga abort's `findNonDeletedSagaAggregate` query executes, making the aggregate invisible to the abort path. Copy-on-write keeps the original managed entity clean, so the abort query succeeds. Topic's original copy-on-write implementation (copied from the reference) was in fact correct; the "fix" that changed it to in-place was itself the mistake. Course and User must also be corrected to copy-on-write.
