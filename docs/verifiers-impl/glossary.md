...
| ScenarioExecutor materializability | Workload-level v3 policy requiring every accepted participant input to pass current readiness checks and the workload to be structurally admissible. Structural admissibility rejects repeated `(sagaInstanceId, runtimeStepName)` forward occurrences until Saga/local runtime state becomes occurrence-aware; the same name across different participants is valid. It permits eager FaultScenario generation but predicts neither domain success nor exact runtime conformance. |
