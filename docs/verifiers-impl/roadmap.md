...
| Synchronous UpdateQuestionTopics extraction gap | C | Diagnosed 2026-09-06: `each` closures are not traversed, so the call is absent before state/adapter filtering. Loop-produced Topic ids and fresh Question identities also need unsupported iteration/accumulation semantics. No coverage gain or Docker target; next choose a direct synchronous application test with supported preparation, or separately approve bounded loop expansion. See current-state closure diagnosis. The broader 32-Saga classification is complete. |
...
| Startup/build cost and batch orchestration | D | [Prepared batch execution](../../verifiers/experiments/batch-execution/README.md) supports explicit frozen selections, repetitions, per-attempt timeout and concurrency 1/2. Six known cases provide a bounded cost sample; compilation/generation cost and broader-workload budgets remain separate. |
