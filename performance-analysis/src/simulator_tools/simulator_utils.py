...
class SimInterface:
    ...
    def start():
        """Start tracing."""
        SimInterface._get("traces/start")
        SimInterface._get("scheduler/start")

    @staticmethod
    def stop():
        """Stops and flushes tracing."""
        SimInterface._get("scheduler/stop")
        SimInterface._get("traces/end")
    ...
    def create_user(name=None, username=None, role="STUDENT", client=requests):
        ...
        r = SimInterface._post("/users/create", json=payload, client=client)
    ...
    def get_user(user_id, client=requests):
        r = SimInterface._get(f"/users/{user_id}", client=client)
    ...
    def activate_user(user_id, client=requests):
        r = SimInterface._post(f"/users/{user_id}/activate", client=client)
    ...
    def create_execution(name=None, acronym=None, academic_term=None, end_date=None, client=requests):
        ...
        r = SimInterface._post("executions/create",
    ...
    def get_execution(execution_id, client=requests):
        r = SimInterface._get(f"/executions/{execution_id}", client=client)
        if r is not None and r.status_code == 200:
            return r.json()
        return None
    ...
    def create_tournament(execution_id, topic_ids, user_id=None, num_questions=2, client=requests):
        if not user_id:
            user_id = SimInterface.create_and_activate_user(client=client)
            if not user_id:
                return None
            SimInterface.enroll_student(user_id, execution_id, client=client)
        ...
        r = SimInterface._post(f"/executions/{execution_id}/tournaments/create", json=payload, params={
    ...
    def update_tournament(tournament_id, topic_ids, num_questions=3, client=requests):
        ...
        r = SimInterface._post("/tournaments/update", json=payload,
    ...
    def solve_quiz(tournament_id, user_id, client=requests):
        r = SimInterface._post(f"/tournaments/{tournament_id}/solveQuiz", params={
    ...
    def get_students(client=requests):
        r = SimInterface._get("/users/students", client=client)
        if r is not None and r.status_code == 200:
            return r.json()
        return None
    ...
    def create_base_data(client=requests):
        ...
        exec_data = SimInterface.create_execution(client=client)
        if not exec_data:
            logging.error("create_base_data aborted: create_execution failed.")
            return None

        course_id = exec_data.get("courseAggregateId")
        topic_data = SimInterface.create_topic(course_id, client=client)
        if not topic_data:
            logging.error("create_base_data aborted: create_topic failed.")
            return None

        SimInterface.create_questions(course_id, topic_data, client=client)
    ...
    def create_and_activate_user(client=requests):
        ...
        user_data = SimInterface.create_user(client=client)

        if user_data:
            user_id = user_data.get("aggregateId")
            if user_id and SimInterface.activate_user(user_id, client=client):
                return user_id
    ...
    def enroll_and_create_tournament(exec_id, user_id, topic_id, client=requests):
        ...
        if SimInterface.enroll_student(user_id, exec_id, client=client):
            return SimInterface.create_tournament(exec_id, [topic_id], user_id, client=client)
