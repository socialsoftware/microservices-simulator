...
@ControllerAdvice
public class ShowcaseExceptionHandler {
    private static final Logger logger = LoggerFactory.getLogger(ShowcaseExceptionHandler.class);

    @ExceptionHandler(ShowcaseException.class)
    public ResponseEntity<String> handleShowcaseException(ShowcaseException ex) {
        logger.warn("Handled ShowcaseException: {}", ex.getErrorMessage());
