...
|-----------|-------|---------------------|
| RULE_A    | L1    | intra-invariant in verifyInvariants() |
| RULE_B    | L2    | service guard in XxxService |
| RULE_C    | L3    | setForbiddenStates in XxxFunctionalitySagas, step "stepName" |
| RULE_D    | L4    | inter-invariant — event subscription chain |
...
- `RULE_C` → `CreateXxxFunctionalitySagas`, step `"getYyy"`
...
- `docs/concepts/decision-guide.md` — the classification flowchart
...
3. For each aggregate in order, fill the Aggregate Details section: write/read functionalities
   ...
   Layer 3 rules with their step names, and the full file list per session.
...
**Reads:**
- `plan.md` — aggregate N section (write functionalities list, Layer 3 rules with step names)
...
**Produces:**
- `{Aggregate}Service.java` — one method per write functionality; service guards (Layer 2) here
...
- One `{Op}FunctionalitySagas.java` per write operation — saga steps with `setForbiddenStates`
  for every Layer 3 rule on that step
...
**Reads:**
- `plan.md` — aggregate N section (events subscribed, Layer 4 rules)
