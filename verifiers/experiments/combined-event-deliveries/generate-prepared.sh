...
java -cp "$cp_value" pt.ulisboa.tecnico.socialsoftware.ms.verifiers.faults.ScenarioGeneratorApplication \
  ...
  --verifiers.scenario-catalog.max-saga-set-size=1 --verifiers.scenario-catalog.max-catalog-scenarios=10000 \
  --verifiers.scenario-catalog.max-input-variants-per-saga=1000 \
  --verifiers.scenario-catalog.max-schedules-per-input-tuple=20 \
  --verifiers.scenario-catalog.max-event-consequences-per-workload=3 \
  --verifiers.scenario-catalog.recovery-schedule-cap=20 \
