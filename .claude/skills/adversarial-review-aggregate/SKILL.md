...
| R4 | Classify each step that mutates an aggregate as primary, foreign or newly-created per `docs/concepts/sagas.md` § R4 Decision Table — all three rows, not two. Primary (the pre-existing aggregate owning this saga) → the preceding lock step must use `SagaCommand` + `setSemanticLock`, and the mutating step sends a plain unwrapped command. Foreign (pre-existing upstream aggregate touched by a cross-aggregate step) → the step itself must declare `setForbiddenStates([...])`. Newly created (the step brings the aggregate into existence) → it declares **neither**, and demanding either is a false violation; it is a defect only when a later step follows it and it registered no compensation to remove what it created (`docs/concepts/sagas.md` § "Create Functionality Sagas"). A primary mutation step with no `setForbiddenStates` is not a violation. |
